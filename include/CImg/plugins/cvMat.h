...
CImg(const cimg_library::CImg<unsigned char>& src):_width(0),_height(0),_depth(0),_spectrum(0),_is_shared(false),_data(0) {
  assign(src);
}

// Conversion IPL -> CImg (in-place constructor)
CImg<T>& assign(const cimg_library::CImg<unsigned char> & src) {
  if (src.isContinuous()) {
    switch (src.depth()) {
    ...
    default:
      throw CImgInstanceException(_cimg_instance
                                  "assign(const cimg_library::CImg<unsigned char>&) : Mat depth is invalid.",
                                  cimg_instance);
      break;
    }
  } else {
...
// Conversion CImg -> MAT
cimg_library::CImg<unsigned char> get_MAT(const unsigned int z=0) const {
  if (is_empty())
    throw CImgInstanceException(_cimg_instance
  ...
  std::vector<cimg_library::CImg<unsigned char>> channels(nchannels);
  if (nchannels>1) {
    for (int c = 0; c<nchannels; ++c) {
      channels[c] = cimg_library::CImg<unsigned char>(rows,cols,matType,const_cast<T*>(buf.data() + rows*cols*(nchannels - 1 - c)));
    } // for channels
    cv::merge(channels,out);
  } else out = cimg_library::CImg<unsigned char>(rows,cols,matType,const_cast<T*>(buf.data())).clone();
  return out;
}
